...
// [assembly: WebActivatorEx.PostApplicationStartMethod(typeof(SitecoreSite.App_Start.GlassMapperSc), "Start")]

namespace SitecoreSite.App_Start
{
	public class GlassMapperSc
	{
        ...
        public static void LoadConfigurationMaps(Glass.Mapper.Sc.IoC.IDependencyResolver resolver, Glass.Mapper.Context context)
        {
            var dependencyResolver = resolver as Glass.Mapper.Sc.IoC.DependencyResolver;
            if (dependencyResolver == null)
            {
                return;
            }

            if (dependencyResolver.ConfigurationMapFactory is Glass.Mapper.Sc.IoC.ConfigurationMapConfigFactory)
            {
